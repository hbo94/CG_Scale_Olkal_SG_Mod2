...
void setup(void)
{
  ...
  unsigned long stabilisingtime = 3000;
  ...
  // EEPROM.get(adrCGoffset, CGoffset); // CG offset, change with type of support
  ...
  // starting the scales
  toOLED("BALANCE CG", "", "Initialisation", "en cours ...");
  ScaleFront.begin(); //start the Scales
  ScaleBack.begin();
  ScaleFront.start(2500); //Stabilize Scales
  ScaleBack.start(2500);
  ...
  while (setupIsFinish != 1) {
    ...
    switch (numMenu) {
      ...
      case 4: // tare definition menu
        toOLED("Valeur tare :", str_weight, "increment:", str_incr);
        break;
  ...
  toOLED("Configuration", "terminee.", "Pret pour peser", "un modele.");
  delay(2000);
...
void loop(void) {
  ...
  if (t1 < millis()) {  //start calculation
    ...
    if (weightTot < -100)
      sprintf(weightLCD, "Erreur");
    else
      sprintf (weightLCD, "Poids: %ld g", weightTot / 100);

    if (CG != 0)
      sprintf(CGtoLCD, "CG: %ld mm", CG / 100);
    else
      sprintf(CGtoLCD, "Pas de CG");

    // Battery value display
    int batVal = readBattVoltage(&bBatWarning);
    if (bBatWarning)
      sprintf(batToLCD, "Bat faible: %d.%d V", (batVal / 1000), ((batVal % 1000) / 100), ((batVal % 100) / 10));
    else
...
void clickHandler(int key) {
  ...
  if (numMenu == 2) {
    ...
    switch (key) {
      ...
      case 4: // Enter
        ...
        //EEPROM.put(adrCGoffset, CGoffset); // save it for each new start of the scale without altering the support
        // numMenu = 1;
    }
  };
    ...
    switch (key) {
      ...
      case 3: // Right
        ...
        if (selectedItem == 2) { // Front calibration procedure
          toOLED("Poser la tare sur", "la balance avant", "'Enter' 5s", "pour lancer.");
          numMenu = 5;
        };
        if (selectedItem == 3) { // Back calibration procedure
          toOLED("Poser la tare sur", "la balance arriere", "'Enter' 5s", "pour lancer.");
          numMenu = 6;
        };
        break;
  ...
  if (numMenu == 4) {     // Tare weight definition menu
    ...
    switch (key) {
      ...
      case 4: // Enter
        //weight = weight;
        //toOLED("Nouveau poids", "de reference", digit1, "");
        //delay(3000);
        // numMenu = 3;
        break;
    } // end switch key
...
void pressHandler(int key) {
  ...
  if (numMenu == 4 && key == 4) {
    toOLED("Nouveau poids", "de la tare", str_weight, "");
    ...
    switch (key) {
      ...
      case 4: // Enter
        ...
        if (SF_front != SF_front_default) {
          ...
          toOLED("Balance avant", "calibration", "terminee.", "");
          calibFrontOK = 1;
  ...
  if (numMenu == 6) {   // menu 6 => back scale calibration procedure
    ...
    switch (key) {
      ...
      case 4: // Enter
        ...
        while (abs(SW_back - weight) >= 0.50) {
          ScaleBack.update();
          delay(200);
          diff = abs(SW_back - weight);
          if (i == 0) { //just to see on display that programm is not hanging
            toOLED("Balance arriere", "", "calibration", "en cours  |");
            toOLED("Balance arriere", "", "calibration", "en cours  /");
            i = 1;
          }
          else {
        ...
        if (SF_back != SF_back_default) {
          ...
          toOLED("Balance arriere", "calibration", "terminee.", "");
          calibBackOK = 1;
...
void displayMenu(byte num, byte selectedItem) {
  ...
  char const *menuItem20 = " Choix support";
  char const *menuItem30 = " Calibration";
  char const *menuItem21 = "Support : ";
  char const *menuItem22 = " Petit";
  char const *menuItem23 = " Moyen";
  char const *menuItem24 = " Grand";
  char const *menuItem31 = " Config. tare";
  char const *menuItem32 = " Calib. avant";
  char const *menuItem33 = " Calib. arriere";
  char const *menuItem40 = " Quitter";
  char selected[20] = {""};
  char support[20] = {""};
  char selector[] = {'>'};
  char r1[22] ;
  ...
  if (supportSelected == 1) {
    ...
    strcat(support, "petit");
  }
  if (supportSelected == 2) {
    strcpy(support, menuItem21);
    strcat(support, "moyen");
  }
  if (supportSelected == 3) {
    strcpy(support, menuItem21);
    strcat(support, "grand");
  }
